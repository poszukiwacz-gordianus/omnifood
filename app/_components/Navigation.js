...
export default function Navigation({ sticky }) {
  ...
  return (
    <>
      <nav
        className={`${isOpen ? "pointer-events-auto visible translate-x-0 bg-primary-100 bg-opacity-85" : "invisible translate-x-full opacity-0"} absolute left-0 top-0 flex h-screen w-full items-center justify-center transition-all duration-1000 ease-in-out lg:visible lg:static lg:h-auto lg:w-auto lg:translate-x-0 lg:items-start lg:justify-start lg:opacity-100`}
      >
